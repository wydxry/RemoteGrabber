...
def files_transfer_filter(is_download, local_folder, remote_folder, local_file_list = None, remote_file_list=None):
    ...
    """
    print("files_transfer_filter begin")
    ...
    if is_download:
        ...
        with open(path, "r") as file:
            ...
            for i in range(0, len(lines), 40):
                ...
                assert len(fpaths) == 9
                filename = fpaths[-1]
                path_prefix = fpaths[-4] + "/" + fpaths[-3] + "/" + fpaths[-2] + "/"
        ...
        for i in range(min(400, len(all_path))):
            ...
            tasks.append((os.path.join(local_folder + all_path[i][0], all_path[i][1]), os.path.join(remote_folder + all_path[i][0], all_path[i][1])))
            tasks.append((os.path.join(local_folder + all_path[i][0], all_path[i][2]), os.path.join(remote_folder + all_path[i][0], all_path[i][2])))
    else:
    ...
    print("files_transfer_filter end")
    return tasks
